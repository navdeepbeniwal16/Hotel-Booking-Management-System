...
import { Container, Card, ListGroup, ListGroupItem, Button, Spinner } from 'react-bootstrap';
...
const BookingsList = () => {
              ...
              <ListGroupItem as="li" key={booking.id}>
                <Card>
                  ...
                  <Card.Footer>
                    <Button
                      variant="primary"
                      onClick={() => navigate(`${booking.id}`)}>Edit</Button>
                  </Card.Footer>
                </Card>
              </ListGroupItem>
            ))}
  ...
  return (
    <>
      {
        loading ? <Spinner animation="border" /> : renderList()
      }
    </>
  );
};
